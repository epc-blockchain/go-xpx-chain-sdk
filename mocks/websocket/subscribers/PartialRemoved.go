// Code generated by mockery v1.0.0. DO NOT EDIT.

package mocks

import mock "github.com/stretchr/testify/mock"
import sdk "github.com/epc-blockchain/go-xpx-chain-sdk/sdk"
import subscribers "github.com/epc-blockchain/go-xpx-chain-sdk/sdk/websocket/subscribers"

// PartialRemoved is an autogenerated mock type for the PartialRemoved type
type PartialRemoved struct {
	mock.Mock
}

// AddHandlers provides a mock function with given fields: address, handlers
func (_m *PartialRemoved) AddHandlers(address *sdk.Address, handlers ...subscribers.PartialRemovedHandler) error {
	_va := make([]interface{}, len(handlers))
	for _i := range handlers {
		_va[_i] = handlers[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, address)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 error
	if rf, ok := ret.Get(0).(func(*sdk.Address, ...subscribers.PartialRemovedHandler) error); ok {
		r0 = rf(address, handlers...)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// GetAddresses provides a mock function with given fields:
func (_m *PartialRemoved) GetAddresses() []string {
	ret := _m.Called()

	var r0 []string
	if rf, ok := ret.Get(0).(func() []string); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]string)
		}
	}

	return r0
}

// GetHandlers provides a mock function with given fields: address
func (_m *PartialRemoved) GetHandlers(address *sdk.Address) []*subscribers.PartialRemovedHandler {
	ret := _m.Called(address)

	var r0 []*subscribers.PartialRemovedHandler
	if rf, ok := ret.Get(0).(func(*sdk.Address) []*subscribers.PartialRemovedHandler); ok {
		r0 = rf(address)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*subscribers.PartialRemovedHandler)
		}
	}

	return r0
}

// HasHandlers provides a mock function with given fields: address
func (_m *PartialRemoved) HasHandlers(address *sdk.Address) bool {
	ret := _m.Called(address)

	var r0 bool
	if rf, ok := ret.Get(0).(func(*sdk.Address) bool); ok {
		r0 = rf(address)
	} else {
		r0 = ret.Get(0).(bool)
	}

	return r0
}

// RemoveHandlers provides a mock function with given fields: address, handlers
func (_m *PartialRemoved) RemoveHandlers(address *sdk.Address, handlers ...*subscribers.PartialRemovedHandler) bool {
	_va := make([]interface{}, len(handlers))
	for _i := range handlers {
		_va[_i] = handlers[_i]
	}
	var _ca []interface{}
	_ca = append(_ca, address)
	_ca = append(_ca, _va...)
	ret := _m.Called(_ca...)

	var r0 bool
	if rf, ok := ret.Get(0).(func(*sdk.Address, ...*subscribers.PartialRemovedHandler) bool); ok {
		r0 = rf(address, handlers...)
	} else {
		r0 = ret.Get(0).(bool)
	}

	return r0
}
